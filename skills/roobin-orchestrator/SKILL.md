...
| Skill | When to Delegate | Activation |
|-------|------------------|------------|
| `roobin-planner` | Complex task needs structured plan | "Ative roobin-planner e execute *create-plan" |
| `roobin-dev` | Implement code changes | "Ative roobin-dev e execute *develop {task-id}" |
| `roobin-reviewer` | Code review after implementation | "Ative roobin-reviewer e execute *review {task-id}" |
| `roobin-ops` | Git push, PR, releases | "Ative roobin-ops e execute *pre-push" |
| `roobin-pm` | Backlog management, PRD | "Ative roobin-pm e execute *draft" |
...
**CRITICAL:** Subagents have NO conversation history. Always include:

```markdown
Ative a skill {skill-name} lendo ~/.claude/skills/{skill-name}/SKILL.md

Task: {task_description}
Task/Subtask ID: {id} (if applicable)
...
1. Delegate to roobin-planner:
   "Ative roobin-planner. Criar plano para '{task_description}'.
    Gerar subtasks no Roobin MCP via manage_tasks."
...
2. Delegate in parallel:
   Task tool 1: "Ative roobin-dev. Implementar subtask {A}. [context]"
   Task tool 2: "Ative roobin-dev. Implementar subtask {B}. [context]"
   Task tool 3: "Ative roobin-dev. Implementar subtask {C}. [context]"
...
1. Delegate to roobin-reviewer:
   "Ative roobin-reviewer. *review {task-id}"
